/**
 * @fileoverview This file is generated by the Angular 2 template compiler.
 * Do not edit.
 * @suppress {suspiciousCode,uselessCode,missingProperties}
 */
 /* tslint:disable */

import * as import0 from './task-menu.component';
import * as import1 from '@angular/core/src/linker/view';
import * as import2 from '@angular/core/src/linker/debug_context';
import * as import3 from '@angular/core/src/render/api';
import * as import4 from '@angular/core/src/linker/view_utils';
import * as import5 from '@angular/core/src/metadata/view';
import * as import6 from '@angular/core/src/linker/view_type';
import * as import7 from '@angular/core/src/change_detection/constants';
import * as import8 from '@angular/core/src/linker/component_factory';
import * as import9 from '@angular/material/core/compatibility/compatibility';
import * as import10 from '@angular/material/button/button';
import * as import11 from '@angular/material/menu/menu-trigger';
import * as import12 from '@angular/material/icon/icon';
import * as import13 from '@angular/material/menu/menu-directive';
import * as import14 from '../../../contacts/components/assignment-selector/assignment-selector.component';
import * as import15 from '../../../client-accounts/components/assignment-selector/assignment-selector.component';
import * as import16 from '../../../team/components/assignment-selector/assignment-selector.component';
import * as import17 from '@angular/material/menu/menu-item';
import * as import18 from '@angular/core/src/linker/view_container';
import * as import19 from '../../../../node_modules/@angular/material/core/compatibility/compatibility.ngfactory';
import * as import20 from '../../../../node_modules/@angular/material/button/button.ngfactory';
import * as import21 from '../../../../node_modules/@angular/material/menu/menu-trigger.ngfactory';
import * as import22 from '../../../../node_modules/@angular/material/icon/icon.ngfactory';
import * as import23 from '../../../../node_modules/@angular/material/menu/menu-directive.ngfactory';
import * as import24 from '@angular/core/src/linker/query_list';
import * as import25 from '../../../contacts/components/assignment-selector/assignment-selector.component.ngfactory';
import * as import26 from '../../../client-accounts/components/assignment-selector/assignment-selector.component.ngfactory';
import * as import27 from '../../../team/components/assignment-selector/assignment-selector.component.ngfactory';
import * as import28 from '../../../../node_modules/@angular/material/menu/menu-item.ngfactory';
import * as import29 from '@angular/core/src/linker/element_ref';
import * as import30 from '@angular/material/core/overlay/overlay';
import * as import31 from '@angular/material/core/rtl/dir';
import * as import32 from '@angular/material/icon/icon-registry';
import * as import33 from '../../../contacts/contacts.sandbox';
import * as import34 from '../../../client-accounts/client-accounts.sandbox';
import * as import35 from '../../../team/team.sandbox';
export class Wrapper_TaskMenuComponent {
  /*private*/ _eventHandler:Function;
  context:import0.TaskMenuComponent;
  /*private*/ _changed:boolean;
  subscription0:any;
  subscription1:any;
  subscription2:any;
  subscription3:any;
  subscription4:any;
  subscription5:any;
  subscription6:any;
  constructor() {
    this._changed = false;
    this.context = new import0.TaskMenuComponent();
  }
  ngOnDetach(view:import1.AppView<any>,componentView:import1.AppView<any>,el:any):void {
  }
  ngOnDestroy():void {
    (this.subscription0 && this.subscription0.unsubscribe());
    (this.subscription1 && this.subscription1.unsubscribe());
    (this.subscription2 && this.subscription2.unsubscribe());
    (this.subscription3 && this.subscription3.unsubscribe());
    (this.subscription4 && this.subscription4.unsubscribe());
    (this.subscription5 && this.subscription5.unsubscribe());
    (this.subscription6 && this.subscription6.unsubscribe());
  }
  ngDoCheck(view:import1.AppView<any>,el:any,throwOnChange:boolean):boolean {
    var changed:any = this._changed;
    this._changed = false;
    return changed;
  }
  checkHost(view:import1.AppView<any>,componentView:import1.AppView<any>,el:any,throwOnChange:boolean):void {
  }
  handleEvent(eventName:string,$event:any):boolean {
    var result:boolean = true;
    return result;
  }
  subscribe(view:import1.AppView<any>,_eventHandler:any,emit0:boolean,emit1:boolean,emit2:boolean,emit3:boolean,emit4:boolean,emit5:boolean,emit6:boolean):void {
    this._eventHandler = _eventHandler;
    if (emit0) { (this.subscription0 = this.context.updateContent.subscribe(_eventHandler.bind(view,'updateContent'))); }
    if (emit1) { (this.subscription1 = this.context.updateContact.subscribe(_eventHandler.bind(view,'updateContact'))); }
    if (emit2) { (this.subscription2 = this.context.updateClientAccount.subscribe(_eventHandler.bind(view,'updateClientAccount'))); }
    if (emit3) { (this.subscription3 = this.context.updateSale.subscribe(_eventHandler.bind(view,'updateSale'))); }
    if (emit4) { (this.subscription4 = this.context.updateAssignee.subscribe(_eventHandler.bind(view,'updateAssignee'))); }
    if (emit5) { (this.subscription5 = this.context.updateDueDate.subscribe(_eventHandler.bind(view,'updateDueDate'))); }
    if (emit6) { (this.subscription6 = this.context.removeTask.subscribe(_eventHandler.bind(view,'removeTask'))); }
  }
}
const nodeDebugInfos_TaskMenuComponent_Host0:import2.StaticNodeDebugInfo[] = [new import2.StaticNodeDebugInfo([import0.TaskMenuComponent],import0.TaskMenuComponent,{})];
var renderType_TaskMenuComponent_Host:import3.RenderComponentType = import4.createRenderComponentType('',0,import5.ViewEncapsulation.None,([] as any[]),{});
class View_TaskMenuComponent_Host0 extends import1.DebugAppView<any> {
  _el_0:any;
  compView_0:import1.AppView<import0.TaskMenuComponent>;
  _TaskMenuComponent_0_3:Wrapper_TaskMenuComponent;
  constructor(viewUtils:import4.ViewUtils,parentView:import1.AppView<any>,parentIndex:number,parentElement:any) {
    super(View_TaskMenuComponent_Host0,renderType_TaskMenuComponent_Host,import6.ViewType.HOST,viewUtils,parentView,parentIndex,parentElement,import7.ChangeDetectorStatus.CheckAlways,nodeDebugInfos_TaskMenuComponent_Host0);
  }
  createInternal(rootSelector:string):import8.ComponentRef<any> {
    this._el_0 = import4.selectOrCreateRenderHostElement(this.renderer,'task-menu',import4.EMPTY_INLINE_ARRAY,rootSelector,this.debug(0,0,0));
    this.compView_0 = new View_TaskMenuComponent0(this.viewUtils,this,0,this._el_0);
    this._TaskMenuComponent_0_3 = new Wrapper_TaskMenuComponent();
    this.compView_0.create(this._TaskMenuComponent_0_3.context);
    this.init(this._el_0,((<any>this.renderer).directRenderer? (null as any): [this._el_0]),(null as any));
    return new import8.ComponentRef_<any>(0,this,this._el_0,this._TaskMenuComponent_0_3.context);
  }
  injectorGetInternal(token:any,requestNodeIndex:number,notFoundResult:any):any {
    if (((token === import0.TaskMenuComponent) && (0 === requestNodeIndex))) { return this._TaskMenuComponent_0_3.context; }
    return notFoundResult;
  }
  detectChangesInternal(throwOnChange:boolean):void {
    this.debug(0,0,0);
    if (this._TaskMenuComponent_0_3.ngDoCheck(this,this._el_0,throwOnChange)) { this.compView_0.markAsCheckOnce(); }
    this.compView_0.internalDetectChanges(throwOnChange);
  }
  destroyInternal():void {
    this.compView_0.destroy();
    this._TaskMenuComponent_0_3.ngOnDestroy();
  }
  visitRootNodesInternal(cb:any,ctx:any):void {
    cb(this._el_0,ctx);
  }
}
export const TaskMenuComponentNgFactory:import8.ComponentFactory<import0.TaskMenuComponent> = new import8.ComponentFactory<import0.TaskMenuComponent>('task-menu',View_TaskMenuComponent_Host0,import0.TaskMenuComponent);
const styles_TaskMenuComponent:any[] = ([] as any[]);
const nodeDebugInfos_TaskMenuComponent0:import2.StaticNodeDebugInfo[] = [
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo([
    import9.MdPrefixRejector,
    import10.MdButton,
    import10.MdButtonCssMatStyler,
    import11.MdMenuTrigger
  ]
  ,import10.MdButton,{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo([
    import9.MdPrefixRejector,
    import12.MdIcon
  ]
  ,import12.MdIcon,{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo([
    import9.MdPrefixRejector,
    import13.MdMenu
  ]
  ,import13.MdMenu,{taskMenu: import13.MdMenu}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo([import14.ContactAssignmentSelectorComponent],import14.ContactAssignmentSelectorComponent,{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo([import15.ClientAccountAssignmentSelectorComponent],import15.ClientAccountAssignmentSelectorComponent,{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo([import16.TeamAssignmentSelectorComponent],import16.TeamAssignmentSelectorComponent,{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo([import17.MdMenuItem],import17.MdMenuItem,{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{}),
  new import2.StaticNodeDebugInfo(([] as any[]),(null as any),{})
]
;
var renderType_TaskMenuComponent:import3.RenderComponentType = import4.createRenderComponentType('/Users/jonoforbes/Developer/Ether/ether-app/src/tasks/components/task-menu/task-menu.component.ts class TaskMenuComponent - inline template',0,import5.ViewEncapsulation.None,styles_TaskMenuComponent,{});
export class View_TaskMenuComponent0 extends import1.DebugAppView<import0.TaskMenuComponent> {
  _text_0:any;
  _el_1:any;
  /*private*/ _vc_1:import18.ViewContainer;
  compView_1:import1.AppView<import10.MdButton>;
  _MdPrefixRejector_1_5:import19.Wrapper_MdPrefixRejector;
  _MdButton_1_6:import20.Wrapper_MdButton;
  _MdButtonCssMatStyler_1_7:import20.Wrapper_MdButtonCssMatStyler;
  _MdMenuTrigger_1_8:import21.Wrapper_MdMenuTrigger;
  _text_2:any;
  _el_3:any;
  compView_3:import1.AppView<import12.MdIcon>;
  _MdPrefixRejector_3_3:import19.Wrapper_MdPrefixRejector;
  _MdIcon_3_4:import22.Wrapper_MdIcon;
  _text_4:any;
  _text_5:any;
  _text_6:any;
  _el_7:any;
  compView_7:import1.AppView<import13.MdMenu>;
  _MdPrefixRejector_7_3:import19.Wrapper_MdPrefixRejector;
  _MdMenu_7_4:import23.Wrapper_MdMenu;
  _query_MdMenuItem_7_0:import24.QueryList<any>;
  _text_8:any;
  _el_9:any;
  compView_9:import1.AppView<import14.ContactAssignmentSelectorComponent>;
  _ContactAssignmentSelectorComponent_9_3:import25.Wrapper_ContactAssignmentSelectorComponent;
  _text_10:any;
  _el_11:any;
  compView_11:import1.AppView<import15.ClientAccountAssignmentSelectorComponent>;
  _ClientAccountAssignmentSelectorComponent_11_3:import26.Wrapper_ClientAccountAssignmentSelectorComponent;
  _text_12:any;
  _el_13:any;
  compView_13:import1.AppView<import16.TeamAssignmentSelectorComponent>;
  _TeamAssignmentSelectorComponent_13_3:import27.Wrapper_TeamAssignmentSelectorComponent;
  _text_14:any;
  _el_15:any;
  compView_15:import1.AppView<import17.MdMenuItem>;
  _MdMenuItem_15_3:import28.Wrapper_MdMenuItem;
  _el_16:any;
  _text_17:any;
  _text_18:any;
  _text_19:any;
  constructor(viewUtils:import4.ViewUtils,parentView:import1.AppView<any>,parentIndex:number,parentElement:any) {
    super(View_TaskMenuComponent0,renderType_TaskMenuComponent,import6.ViewType.COMPONENT,viewUtils,parentView,parentIndex,parentElement,import7.ChangeDetectorStatus.CheckOnce,nodeDebugInfos_TaskMenuComponent0);
  }
  createInternal(rootSelector:string):import8.ComponentRef<any> {
    const parentRenderNode:any = this.renderer.createViewRoot(this.parentElement);
    this._text_0 = this.renderer.createText(parentRenderNode,'\n        ',this.debug(0,0,0));
    this._el_1 = import4.createRenderElement(this.renderer,parentRenderNode,'button',new import4.InlineArray8(6,'aria-haspopup','true','md-button','','type','button'),this.debug(1,1,8));
    this._vc_1 = new import18.ViewContainer(1,(null as any),this,this._el_1);
    this.compView_1 = new import20.View_MdButton0(this.viewUtils,this,1,this._el_1);
    this._MdPrefixRejector_1_5 = new import19.Wrapper_MdPrefixRejector(this.parentView.injectorGet(import9.MATERIAL_COMPATIBILITY_MODE,this.parentIndex,(null as any)));
    this._MdButton_1_6 = new import20.Wrapper_MdButton(new import29.ElementRef(this._el_1),this.renderer);
    this._MdButtonCssMatStyler_1_7 = new import20.Wrapper_MdButtonCssMatStyler();
    this._MdMenuTrigger_1_8 = new import21.Wrapper_MdMenuTrigger(this.parentView.injectorGet(import30.Overlay,this.parentIndex),new import29.ElementRef(this._el_1),this._vc_1.vcRef,this.renderer,this.parentView.injectorGet(import31.Dir,this.parentIndex,(null as any)));
    this._text_2 = this.renderer.createText((null as any),'\n            ',this.debug(2,4,26));
    this._el_3 = import4.createRenderElement(this.renderer,(null as any),'md-icon',new import4.InlineArray4(4,'role','img','style','color: #838383; width: 20px;'),this.debug(3,5,12));
    this.compView_3 = new import22.View_MdIcon0(this.viewUtils,this,3,this._el_3);
    this._MdPrefixRejector_3_3 = new import19.Wrapper_MdPrefixRejector(this.parentView.injectorGet(import9.MATERIAL_COMPATIBILITY_MODE,this.parentIndex,(null as any)));
    this._MdIcon_3_4 = new import22.Wrapper_MdIcon(new import29.ElementRef(this._el_3),this.renderer,this.parentView.injectorGet(import32.MdIconRegistry,this.parentIndex));
    this._text_4 = this.renderer.createText((null as any),'more_horiz',this.debug(4,5,58));
    this.compView_3.create(this._MdIcon_3_4.context);
    this._text_5 = this.renderer.createText((null as any),'\n            \n        ',this.debug(5,5,78));
    this.compView_1.create(this._MdButton_1_6.context);
    this._text_6 = this.renderer.createText(parentRenderNode,'\n        \n        ',this.debug(6,7,17));
    this._el_7 = import4.createRenderElement(this.renderer,parentRenderNode,'md-menu',new import4.InlineArray4(4,'role','menu','x-position','before'),this.debug(7,9,8));
    this.compView_7 = new import23.View_MdMenu0(this.viewUtils,this,7,this._el_7);
    this._MdPrefixRejector_7_3 = new import19.Wrapper_MdPrefixRejector(this.parentView.injectorGet(import9.MATERIAL_COMPATIBILITY_MODE,this.parentIndex,(null as any)));
    this._MdMenu_7_4 = new import23.Wrapper_MdMenu('before',(null as any));
    this._query_MdMenuItem_7_0 = new import24.QueryList<any>();
    this._text_8 = this.renderer.createText((null as any),'\n            ',this.debug(8,9,56));
    this._el_9 = import4.createRenderElement(this.renderer,(null as any),'contact-assignment-selector',import4.EMPTY_INLINE_ARRAY,this.debug(9,10,12));
    this.compView_9 = new import25.View_ContactAssignmentSelectorComponent0(this.viewUtils,this,9,this._el_9);
    this._ContactAssignmentSelectorComponent_9_3 = new import25.Wrapper_ContactAssignmentSelectorComponent(this.parentView.injectorGet(import33.ContactsSandbox,this.parentIndex));
    this.compView_9.create(this._ContactAssignmentSelectorComponent_9_3.context);
    this._text_10 = this.renderer.createText((null as any),'\n            ',this.debug(10,10,113));
    this._el_11 = import4.createRenderElement(this.renderer,(null as any),'account-assignment-selector',import4.EMPTY_INLINE_ARRAY,this.debug(11,11,12));
    this.compView_11 = new import26.View_ClientAccountAssignmentSelectorComponent0(this.viewUtils,this,11,this._el_11);
    this._ClientAccountAssignmentSelectorComponent_11_3 = new import26.Wrapper_ClientAccountAssignmentSelectorComponent(this.parentView.injectorGet(import34.ClientAccountsSandbox,this.parentIndex));
    this.compView_11.create(this._ClientAccountAssignmentSelectorComponent_11_3.context);
    this._text_12 = this.renderer.createText((null as any),'\n            ',this.debug(12,11,144));
    this._el_13 = import4.createRenderElement(this.renderer,(null as any),'team-assignment-selector',import4.EMPTY_INLINE_ARRAY,this.debug(13,12,12));
    this.compView_13 = new import27.View_TeamAssignmentSelectorComponent0(this.viewUtils,this,13,this._el_13);
    this._TeamAssignmentSelectorComponent_13_3 = new import27.Wrapper_TeamAssignmentSelectorComponent(this.parentView.injectorGet(import35.TeamSandbox,this.parentIndex));
    this.compView_13.create(this._TeamAssignmentSelectorComponent_13_3.context);
    this._text_14 = this.renderer.createText((null as any),'\n            ',this.debug(14,12,109));
    this._el_15 = import4.createRenderElement(this.renderer,(null as any),'button',new import4.InlineArray4(4,'md-menu-item','','role','menuitem'),this.debug(15,13,12));
    this.compView_15 = new import28.View_MdMenuItem0(this.viewUtils,this,15,this._el_15);
    this._MdMenuItem_15_3 = new import28.Wrapper_MdMenuItem(this.renderer,new import29.ElementRef(this._el_15));
    this._el_16 = import4.createRenderElement(this.renderer,(null as any),'h1',import4.EMPTY_INLINE_ARRAY,this.debug(16,13,58));
    this._text_17 = this.renderer.createText(this._el_16,'Remove Task',this.debug(17,13,62));
    this.compView_15.create(this._MdMenuItem_15_3.context);
    this._text_18 = this.renderer.createText((null as any),'\n        ',this.debug(18,13,87));
    this.compView_7.create(this._MdMenu_7_4.context);
    this._text_19 = this.renderer.createText(parentRenderNode,'\n    \n    \n    \n    \n    ',this.debug(19,14,18));
    var disposable_0:Function = import4.subscribeToRenderElement(this,this._el_1,new import4.InlineArray8(8,'mousedown',(null as any),'focus',(null as any),'blur',(null as any),'click',(null as any)),this.eventHandler(this.handleEvent_1));
    var disposable_1:Function = import4.subscribeToRenderElement(this,this._el_9,new import4.InlineArray2(2,'updateContact',(null as any)),this.eventHandler(this.handleEvent_9));
    this._ContactAssignmentSelectorComponent_9_3.subscribe(this,this.eventHandler(this.handleEvent_9),true);
    var disposable_2:Function = import4.subscribeToRenderElement(this,this._el_11,new import4.InlineArray2(2,'updateClientAccount',(null as any)),this.eventHandler(this.handleEvent_11));
    this._ClientAccountAssignmentSelectorComponent_11_3.subscribe(this,this.eventHandler(this.handleEvent_11),true);
    var disposable_3:Function = import4.subscribeToRenderElement(this,this._el_13,new import4.InlineArray2(2,'updateAssignee',(null as any)),this.eventHandler(this.handleEvent_13));
    this._TeamAssignmentSelectorComponent_13_3.subscribe(this,this.eventHandler(this.handleEvent_13),true);
    var disposable_4:Function = import4.subscribeToRenderElement(this,this._el_15,new import4.InlineArray2(2,'click',(null as any)),this.eventHandler(this.handleEvent_15));
    this.init((null as any),((<any>this.renderer).directRenderer? (null as any): [
      this._text_0,
      this._el_1,
      this._text_2,
      this._el_3,
      this._text_4,
      this._text_5,
      this._text_6,
      this._el_7,
      this._text_8,
      this._el_9,
      this._text_10,
      this._el_11,
      this._text_12,
      this._el_13,
      this._text_14,
      this._el_15,
      this._el_16,
      this._text_17,
      this._text_18,
      this._text_19
    ]
    ),[
      disposable_0,
      disposable_1,
      disposable_2,
      disposable_3,
      disposable_4
    ]
    );
    return (null as any);
  }
  injectorGetInternal(token:any,requestNodeIndex:number,notFoundResult:any):any {
    if (((token === import9.MdPrefixRejector) && ((3 <= requestNodeIndex) && (requestNodeIndex <= 4)))) { return this._MdPrefixRejector_3_3.context; }
    if (((token === import12.MdIcon) && ((3 <= requestNodeIndex) && (requestNodeIndex <= 4)))) { return this._MdIcon_3_4.context; }
    if (((token === import9.MdPrefixRejector) && ((1 <= requestNodeIndex) && (requestNodeIndex <= 5)))) { return this._MdPrefixRejector_1_5.context; }
    if (((token === import10.MdButton) && ((1 <= requestNodeIndex) && (requestNodeIndex <= 5)))) { return this._MdButton_1_6.context; }
    if (((token === import10.MdButtonCssMatStyler) && ((1 <= requestNodeIndex) && (requestNodeIndex <= 5)))) { return this._MdButtonCssMatStyler_1_7.context; }
    if (((token === import11.MdMenuTrigger) && ((1 <= requestNodeIndex) && (requestNodeIndex <= 5)))) { return this._MdMenuTrigger_1_8.context; }
    if (((token === import14.ContactAssignmentSelectorComponent) && (9 === requestNodeIndex))) { return this._ContactAssignmentSelectorComponent_9_3.context; }
    if (((token === import15.ClientAccountAssignmentSelectorComponent) && (11 === requestNodeIndex))) { return this._ClientAccountAssignmentSelectorComponent_11_3.context; }
    if (((token === import16.TeamAssignmentSelectorComponent) && (13 === requestNodeIndex))) { return this._TeamAssignmentSelectorComponent_13_3.context; }
    if (((token === import17.MdMenuItem) && ((15 <= requestNodeIndex) && (requestNodeIndex <= 17)))) { return this._MdMenuItem_15_3.context; }
    if (((token === import9.MdPrefixRejector) && ((7 <= requestNodeIndex) && (requestNodeIndex <= 18)))) { return this._MdPrefixRejector_7_3.context; }
    if (((token === import13.MdMenu) && ((7 <= requestNodeIndex) && (requestNodeIndex <= 18)))) { return this._MdMenu_7_4.context; }
    return notFoundResult;
  }
  detectChangesInternal(throwOnChange:boolean):void {
    this.debug(1,1,8);
    this._MdPrefixRejector_1_5.ngDoCheck(this,this._el_1,throwOnChange);
    if (this._MdButton_1_6.ngDoCheck(this,this._el_1,throwOnChange)) { this.compView_1.markAsCheckOnce(); }
    this._MdButtonCssMatStyler_1_7.ngDoCheck(this,this._el_1,throwOnChange);
    this.debug(1,3,12);
    const currVal_1_3_0:any = this._MdMenu_7_4.context;
    this._MdMenuTrigger_1_8.check_menu(currVal_1_3_0,throwOnChange,false);
    this._MdMenuTrigger_1_8.ngDoCheck(this,this._el_1,throwOnChange);
    this.debug(3,5,12);
    this._MdPrefixRejector_3_3.ngDoCheck(this,this._el_3,throwOnChange);
    if (this._MdIcon_3_4.ngDoCheck(this,this._el_3,throwOnChange)) { this.compView_3.markAsCheckOnce(); }
    this.debug(7,9,8);
    this._MdPrefixRejector_7_3.ngDoCheck(this,this._el_7,throwOnChange);
    this._MdMenu_7_4.ngDoCheck(this,this._el_7,throwOnChange);
    this.debug(9,10,12);
    if (this._ContactAssignmentSelectorComponent_9_3.ngDoCheck(this,this._el_9,throwOnChange)) { this.compView_9.markAsCheckOnce(); }
    this.debug(11,11,41);
    const currVal_11_0_0:any = true;
    this._ClientAccountAssignmentSelectorComponent_11_3.check_fullTitle(currVal_11_0_0,throwOnChange,false);
    if (this._ClientAccountAssignmentSelectorComponent_11_3.ngDoCheck(this,this._el_11,throwOnChange)) { this.compView_11.markAsCheckOnce(); }
    this.debug(13,12,12);
    if (this._TeamAssignmentSelectorComponent_13_3.ngDoCheck(this,this._el_13,throwOnChange)) { this.compView_13.markAsCheckOnce(); }
    this.debug(15,13,12);
    this._MdMenuItem_15_3.ngDoCheck(this,this._el_15,throwOnChange);
    this._vc_1.detectChangesInNestedViews(throwOnChange);
    if (!throwOnChange) {
      if (this._query_MdMenuItem_7_0.dirty) {
        this._query_MdMenuItem_7_0.reset([this._MdMenuItem_15_3.context]);
        this._MdMenu_7_4.context.items = this._query_MdMenuItem_7_0;
        this._query_MdMenuItem_7_0.notifyOnChanges();
      }
      this.debug(7,9,8);
      if ((this.numberOfChecks === 0)) { this._MdMenu_7_4.context.ngAfterContentInit(); }
    }
    this._MdButton_1_6.checkHost(this,this.compView_1,this._el_1,throwOnChange);
    this._MdButtonCssMatStyler_1_7.checkHost(this,this.compView_1,this._el_1,throwOnChange);
    this._MdIcon_3_4.checkHost(this,this.compView_3,this._el_3,throwOnChange);
    this._MdMenuItem_15_3.checkHost(this,this.compView_15,this._el_15,throwOnChange);
    this.compView_1.internalDetectChanges(throwOnChange);
    this.compView_3.internalDetectChanges(throwOnChange);
    this.compView_7.internalDetectChanges(throwOnChange);
    this.compView_9.internalDetectChanges(throwOnChange);
    this.compView_11.internalDetectChanges(throwOnChange);
    this.compView_13.internalDetectChanges(throwOnChange);
    this.compView_15.internalDetectChanges(throwOnChange);
    if (!throwOnChange) {
      this.debug(3,5,12);
      this._MdIcon_3_4.context.ngAfterViewChecked();
      this.debug(1,1,8);
      if ((this.numberOfChecks === 0)) { this._MdMenuTrigger_1_8.context.ngAfterViewInit(); }
    }
  }
  destroyInternal():void {
    this._vc_1.destroyNestedViews();
    this.compView_1.destroy();
    this.compView_3.destroy();
    this.compView_7.destroy();
    this.compView_9.destroy();
    this.compView_11.destroy();
    this.compView_13.destroy();
    this.compView_15.destroy();
    this.debug(3,5,12);
    this._MdMenuTrigger_1_8.ngOnDestroy();
    this.debug(1,1,8);
    this._ContactAssignmentSelectorComponent_9_3.ngOnDestroy();
    this.debug(9,10,12);
    this._ClientAccountAssignmentSelectorComponent_11_3.ngOnDestroy();
    this.debug(11,11,12);
    this._TeamAssignmentSelectorComponent_13_3.ngOnDestroy();
    this.debug(13,12,12);
    this.debug(15,13,12);
    this._MdMenu_7_4.ngOnDestroy();
  }
  visitProjectableNodesInternal(nodeIndex:number,ngContentIndex:number,cb:any,ctx:any):void {
    if (((nodeIndex == 1) && (ngContentIndex == 0))) {
      cb(this._text_2,ctx);
      cb(this._el_3,ctx);
      cb(this._text_5,ctx);
    }
    if (((nodeIndex == 3) && (ngContentIndex == 0))) { cb(this._text_4,ctx); }
    if (((nodeIndex == 7) && (ngContentIndex == 0))) {
      cb(this._text_8,ctx);
      cb(this._el_9,ctx);
      cb(this._text_10,ctx);
      cb(this._el_11,ctx);
      cb(this._text_12,ctx);
      cb(this._el_13,ctx);
      cb(this._text_14,ctx);
      cb(this._el_15,ctx);
      cb(this._text_18,ctx);
    }
    if (((nodeIndex == 15) && (ngContentIndex == 0))) { cb(this._el_16,ctx); }
  }
  handleEvent_1(eventName:string,$event:any):boolean {
    this.debug(1,1,8);
    this.compView_1.markPathToRootAsCheckOnce();
    var result:boolean = true;
    result = (this._MdButton_1_6.handleEvent(eventName,$event) && result);
    result = (this._MdMenuTrigger_1_8.handleEvent(eventName,$event) && result);
    return result;
  }
  handleEvent_9(eventName:string,$event:any):boolean {
    this.debug(9,10,12);
    this.markPathToRootAsCheckOnce();
    var result:boolean = true;
    if ((eventName == 'updateContact')) {
      const pd_sub_0:any = ((<any>this.context.onUpdateContact($event)) !== false);
      result = (pd_sub_0 && result);
    }
    return result;
  }
  handleEvent_11(eventName:string,$event:any):boolean {
    this.debug(11,11,12);
    this.markPathToRootAsCheckOnce();
    var result:boolean = true;
    if ((eventName == 'updateClientAccount')) {
      const pd_sub_0:any = ((<any>this.context.onUpdateClientAccount($event)) !== false);
      result = (pd_sub_0 && result);
    }
    return result;
  }
  handleEvent_13(eventName:string,$event:any):boolean {
    this.debug(13,12,12);
    this.markPathToRootAsCheckOnce();
    var result:boolean = true;
    if ((eventName == 'updateAssignee')) {
      const pd_sub_0:any = ((<any>this.context.onUpdateAssignee($event)) !== false);
      result = (pd_sub_0 && result);
    }
    return result;
  }
  handleEvent_15(eventName:string,$event:any):boolean {
    this.debug(15,13,12);
    this.compView_15.markPathToRootAsCheckOnce();
    var result:boolean = true;
    result = (this._MdMenuItem_15_3.handleEvent(eventName,$event) && result);
    if ((eventName == 'click')) {
      const pd_sub_0:any = ((<any>this.context.onRemoveTask()) !== false);
      result = (pd_sub_0 && result);
    }
    return result;
  }
}