...
export let TasksWeekContainer = class TasksWeekContainer {
    constructor(sb, route, router) {
        this.sb = sb;
        this.route = route;
        this.router = router;
        this.ngUnsubscribe = new Subject();
        this.headerText = "Tasks for This Week";
    }
    ngOnDestroy() {
        console.log('tasks week destroy');
    ...
    ngOnInit() {
        ...
        let startString = startOfWeek.add(1, 'days').format('Do MMMM YYYY');
        let endString = endOfWeek.format('Do MMMM YYYY');
        this.headerDate = `${startString} - ${endString}`;
        this.matchingTasks$ = this.sb.tasks$
            .takeUntil(this.ngUnsubscribe)
            .map((tasks) => {
            return tasks.filter((task) => {
                return moment(task.dueDate).isBetween(startOfWeek, endOfWeek);
            });
        });
    }
    onSave(task) {
...
//# sourceMappingURL=tasks-week.container.js.map